...
5. After the owner **sends** the TZ+estimate package from the console, the cover letter appears **in the chat thread as a full-width message** with one quoted price (no market fork), hours, file buttons, and **Подтверждаю** / **Отклонить ТЗ** / **Отклонить смету**. There are no comment fields inside the bubble. TZ reject: write a normal chat message afterwards. Estimate reject: a modal comment. The Mini App card is visible only after send (`package_visible`). Planner starts only after confirm ([DEC-012](../decisions/DEC-012-Client-Market-Estimate.md)). Reject does **not** resume the Discovery interviewer. Owner replies appear in the same thread.
...
**Calm:** button **«Спокойный режим»** persists in `localStorage` (`asf-calm-mode`). `prefers-reduced-motion` freezes or hides the mascot and foundry field; status text stays. The companion is the Uni 4 IT unicorn (`apps/miniapp/brand/mascot-bust.png`) with CSS breathe / glance / wave on the same beats; optional `mascot.riv` is still progressive. Logo lockup and mark: `docs/17-Brand-Assets.md`. Lip-sync / TTS mouth shapes are Future.
