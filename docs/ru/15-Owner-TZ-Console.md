...
Внутренний UI **владельца / аналитика** для просмотра собранных требований ТЗ в виде графа и коммерческого цикла ТЗ/сметы. UI заказчика остаётся Telegram Mini App. HITL-утверждение — в боте владельца **и** в консоли. В шапке — wordmark Uni 4 IT (`apps/console/brand/`, см. `docs/17-Brand-Assets.md`).
...
В карточке проекта — **выгрузка полного ТЗ**: Markdown, Word (`docx`), PDF. Файл собирается из KG: `GET /console/api/projects/{id}/tz-export?format=md|docx|pdf`. Клиентский документ — `core/tz_document.compose_tz_markdown`: заголовок **Техническое задание** + имя проекта, мета (проект + контакты заказчика + контакты студии/владельца), оглавление со ссылками, пронумерованные разделы и коды требований `ТЗ-N.M`. Без Appendix и без заголовка «Draft TZ». Контакты исполнителя: `STUDIO_NAME` / `OWNER_CONTACT_*` или хук в KG `Project.payload.owner_contacts` (`{studio, name, email, phone, telegram, note}`). PDF/DOCX — тот же Markdown.

Клик по **центру графа** открывает карточку секциями: **Общее** (название + строка заказчика, сроки/бюджет из KG, derived-пайплайн), **ТЗ**, **Смета**, **MVP**, **Требования**, **Замечания**. Позвоночник — семь уникальных ворот слева направо; версии согласования и MVP растут **колонками вниз**. `ProjectStatus` остаётся FSM Discovery; степпер считается в `core/commercial_pipeline.py`. Клик по шагу показывает **снимок ТЗ** (`GET .../tz-preview`) и diff требований. Можно принудительно шаг назад/вперёд (`POST .../pipeline`) и `PATCH` любой `ProjectStatus` (тест/восстановление). Снимки — JSONB `Project.payload.commercial.tz_snapshots`.

Две оценки: эвристика студии (не цена клиенту) и после HITL — **клиентская смета**. Ставка ₽/ч и скидка %; в письмо уходит `часы × ставка × (1 − %)`. Выгрузка сметы только из сохранённого payload. **Утвердить ТЗ** — тот же HITL. **Отправить ТЗ и смету** — два документа в Telegram плюс карточки в тред; оба файла должны уйти. Ответ владельца без Discovery. Непрочитанное от заказчика — точка в списке проектов. На требованиях — легенда и **дельта с последней отправки пакета**.

Правая панель по умолчанию ≈760px, ширина тянется за ручку (`localStorage`), на узком экране складывается вниз.
...
Показывает: id, **актуальный** текст в редакторе, дату создания, автора (`payload.author_role` / `author_id`), структурного родителя, связи, статус, причину отклонения/конфликта, историю. Правка текста кладёт **полный старый текст** в `entity_history` (`payload.fields.description.from`); в основном поле — новая формулировка.
...
Таблица `entity_history` — журнал аудита (`created`, `updated`, `deleted`, `status_change`, `relation_add`, `relation_remove`). Это **не** event sourcing. Правки текста кладут полное «было» в `payload.fields`. Добавление/удаление файлов кладут имя и этап в `payload`.
