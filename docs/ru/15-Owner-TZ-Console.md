...
Клик по **центру графа** открывает карточку секциями: **Общее** (название + строка заказчика, сроки/бюджет из KG, derived-пайплайн), **ТЗ**, **Смета**, **MVP**, **Требования**, **Замечания**. Позвоночник — семь уникальных ворот слева направо; версии согласования и MVP растут **колонками вниз**. `ProjectStatus` остаётся FSM Discovery; степпер считается в `core/commercial_pipeline.py`. `PATCH` статуса из консоли может только **архивировать**.
