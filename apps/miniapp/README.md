...
- Кнопка **«Спокойный режим»** на экране **Настройки** (не в шапке home / workspace).
- Сохраняется в `localStorage` (`asf-calm-mode=1`).
- `prefers-reduced-motion: reduce` тоже замораживает маскота и Matrix-фон (статичный кадр); текст статуса остаётся.
