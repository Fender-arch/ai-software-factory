...
Imported from [carmahhawwari/ui-design-brain](https://github.com/carmahhawwari/ui-design-brain) (MIT — see `LICENSE.txt`). Use it for **component-level correctness**, not to impose a look:

- **Brand voice wins over the generic "SaaS" default.** ASF chrome (`apps/console`, `apps/miniapp`) and customer MVPs follow `DESIGN.md` + `.cursor/skills/anti-slop-design`. Keep the warm dark + cyan/purple voice; do not "normalize" to neutral SaaS gray, Inter, or a Tailwind default palette.
- `React + Tailwind` in Step 4 is the upstream default — this repo's frontends are hand-written HTML/CSS, and customer repos may vary. Follow the component best practices (states, a11y, touch targets, anti-patterns), not the framework choice.
- `Modern SaaS` is not the default preset here; prefer **Data Dashboard** for the console and **Apple-level Minimal** for the mini app, only where they don't conflict with the ASF tokens.
